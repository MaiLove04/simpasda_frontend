document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderTabel(data) {
        ...
        data.forEach((item, index) => {
            ...
            // Logika Tombol Print PDF Cetak QR Rumah
            ...
            // 🟢 LOGIKA BARU: Tombol Download Gambar QR Code (.png)
            const tombolDownload = item.kode_nasabah
                ? `<button onclick="downloadQR('${item.kode_nasabah}', '${nama.replace(/\s+/g, '_')}')" class="btn btn-sm btn-outline-success px-2 py-1" style="font-size: 11px; font-weight: 600; border-radius: 6px; height: 28px; display: inline-flex; align-items: center; gap: 4px;" title="Download Gambar QR">
                ...
                <tr style="border-bottom: 1px solid #f1f5f9;">
                    ...
                    <td class="py-3 pe-3">
                        <div class="d-flex justify-content-center align-items-center gap-1">
                            ${tombolPrint}
                            ${tombolDownload} <a href="show_nasabah.html?id=${item.id}" class="btn btn-sm text-white px-2 py-1" style="font-size: 11px; font-weight: 600; border-radius: 6px; height: 28px; background-color: #0284c7; border: none; display: inline-flex; align-items: center; gap: 4px;">
...
function ubahStatus(id, statusBaru) {
    const token = localStorage.getItem("token_simpasda");
    const API_STATUS_URL = `http://127.0.0.1:8000/api/nasabah/${id}/status`;
...
// 🟢 7. FUNGSI BARU: Download QR Code Gambar Resolusi Tinggi secara Langsung
function downloadQR(kodeNasabah, namaFile) {
    // Kita request ukuran 300x300 agar gambar QR yang diunduh tajam dan tidak blur
    const urlGambar = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${kodeNasabah}`;
    
    fetch(urlGambar)
        .then(response => response.blob()) // Mengubah response gambar menjadi data mentah blob
        .then(blob => {
            const urlBlob = window.URL.createObjectURL(blob);
            const linkDummy = document.createElement('a'); // Membuat tag <a> virtual
            
            linkDummy.href = urlBlob;
            linkDummy.download = `QR_${namaFile}.png`; // Mengeset nama file unduhan
            
            document.body.appendChild(linkDummy);
            linkDummy.click(); // Memicu klik otomatis unduhan browser
            
            document.body.removeChild(linkDummy);
            window.URL.revokeObjectURL(urlBlob); // Membersihkan memori browser kembali
        })
        .catch(err => {
            console.error("Gagal mengunduh QR Code:", err);
            alert("Terjadi masalah saat mengunduh gambar QR Code.");
        });
}